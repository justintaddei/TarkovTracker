...
export class DataValidationUtils {
   ...
   */
  static validateImportFormat(parsedJson: unknown): parsedJson is { type: string; data: ProgressData } {
    return (
  ...
  static sanitizeProgressData(data: ProgressData): ProgressData {
    return {
      ...
      gameEdition: ['standard', 'leftbehind', 'prepareescape', 'edgeofDarkness'].includes(data.gameEdition || '') 
        ? data.gameEdition 
        : 'standard',
