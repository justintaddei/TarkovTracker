...
<script setup>
  ...
  const callTeamFunction = async (functionName, payload = {}) => {
    try {
      ...
      if (error.message?.includes('CORS') || error.code === 'functions/internal') {
        const idToken = await auth.currentUser.getIdToken();
        const endpoint = functionName.replace('Team', '').toLowerCase();
        const response = await fetch(
          `http://localhost:5001/tarkov-tracker-dev/us-central1/api/api/team/${endpoint}`,
          {
            method: 'POST',
            headers: {
