...
test.describe('Tasks Page', () => {
              ...
              tasks: [
                {
                  ...
                  trader: { name: 'Prapor' },
                  objectives: [
                    { id: 'obj-1', description: 'Test objective', completed: false }
                  ]
                }
              ]
            }
  ...
  test('should search tasks when search functionality exists', async ({ page }) => {
    ...
    const searchInput = page.locator('input[type="text"], .v-text-field input, [placeholder*="search" i]').first()
    ...
    if (await searchInput.isVisible()) {
      ...
      const hasSearchPerformed = currentUrl.includes('search') || bodyText.includes('test') || bodyText.includes('No results')
